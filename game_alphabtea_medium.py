...
class Game:
    ...
    def __init__(self):
        ...
        self.x = 0
        self.y = 0
        self.cpu_move_count = 0
        self.turnCounter = 0
        self.possibleNumbers = [1, 2, 3, 4, 5, 6, 7, 8, 9]
        self.gameBoard = [[' ', ' ', ' '], [' ', ' ', ' '], [' ', ' ', ' ']]
    
    def evaluate_board(self, board):
        x1,y1,x2,y2,winner = self.checkForWinner(board)
        if winner == 'X':
            return 1  
        elif winner == 'O':
            return -1  
        ...
        for i in range(3):
            for j in range(3):
                if self.gameBoard[i][j] == ' ':
                    ...
                    move_val = self.alpha_beta_pruning(self.gameBoard, 0,float('-inf'), float('inf'), False)
                    self.gameBoard[i][j] = ' '  
    ...
    def handle_mouse_click2(self):
        if self.mode=="Player 2":
           ...
           if num in self.possibleNumbers:
            ...
            self.cpu_move_count+=1
            move_no = self.equal = random.choice([2,4,5,6])
            if self.cpu_move_count != move_no: 
                best_move = self.find_best_move()
            else:
                best_move = random.choice([(i, j) for i in range(3) for j in range(3) if self.gameBoard[i][j] == ' '])
            if best_move != None:
    ...
    def check_and_handle_game_result(self):
        x1,y1,x2,y2,winner = self.checkForWinner(self.gameBoard)
        if winner != "N":
